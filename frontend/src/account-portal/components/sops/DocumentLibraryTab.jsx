      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-5 space-y-4">
        <div className="border-b border-slate-100 pb-3">
          <h2 className="text-base font-bold text-slate-800 flex items-center gap-2">
            <Archive className="w-5 h-5 text-[#ff5a36]" /> {title}
          </h2>
          ...
          {acts.map((a) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition border ${
                selectedAct === a ? "bg-[#ff5a36] text-white border-[#ff5a36]" : "bg-slate-50 text-slate-600 border-slate-200 hover:bg-slate-100"
              }`}
            >
              {a}
            </button>
            ...
            <div className="space-y-0.5 lg:max-h-[520px] lg:overflow-y-auto">
              <button
                ...
                className={`w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg text-xs font-bold text-left border-l-2 transition ${
                  activeTypeFilter === "ALL" ? "bg-[#fff1ec] text-[#c8431f] border-[#ff5a36]" : "text-slate-600 border-transparent hover:bg-slate-50"
                }`}
              >
                <span className="truncate">All</span>
                <span className={`text-[9.5px] px-1.5 py-0.5 rounded-full font-extrabold flex-shrink-0 ${activeTypeFilter === "ALL" ? "bg-[#ffe4da] text-[#c8431f]" : "bg-slate-100 text-slate-500"}`}>
                  {files.length}
                </span>
              </button>
              {groups.map((g) => (
                <button
                  ...
                  title={g.name}
                  className={`w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg text-xs font-bold text-left border-l-2 transition ${
                    activeTypeFilter === g.name ? "bg-[#fff1ec] text-[#c8431f] border-[#ff5a36]" : "text-slate-600 border-transparent hover:bg-slate-50"
                  }`}
                >
                  <span className="truncate">{g.name}</span>
                  <span className={`text-[9.5px] px-1.5 py-0.5 rounded-full font-extrabold flex-shrink-0 ${activeTypeFilter === g.name ? "bg-[#ffe4da] text-[#c8431f]" : "bg-slate-100 text-slate-500"}`}>
                    {g.count}
                  </span>
                </button>
              ))}
                  ...
                  {filtered.map((doc) => (
                    <div key={doc.id} className="bg-white rounded-lg border border-slate-200 p-3.5 shadow-2xs hover:border-slate-300 transition flex flex-col justify-between space-y-2.5">
                      <div>
                        <div className="flex justify-between items-start gap-2">
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            {doc.has_file && (
                              <>
                                <button onClick={() => viewSopFile(doc.id)} title="View" className="text-slate-400 hover:text-[#ff5a36] p-1 rounded hover:bg-slate-100 transition">
                      ...
                      <div className="pt-2 border-t border-slate-100 flex flex-wrap gap-1 text-[9px] font-bold">
                        <span className="bg-[#fff1ec] text-[#c8431f] px-1.5 py-0.5 rounded border border-[#ffe4da]">{doc.sop_name}</span>
                        {doc.process_name && (
