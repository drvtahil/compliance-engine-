...
function getCourseStatus(c) {
  ...
  if (c.completed_count < c.content_count) {
    return { label: "In Progress", icon: PlayCircle, badge: "bg-[#fff1ec] text-[#c8431f] border-[#ffc7ae]", bar: "bg-[#ff5a36]" };
  }
        ...
        <div className="flex items-center bg-slate-100 rounded-lg p-0.5">
          {[["courses", "My Courses"], ["results", "My Results"]].map(([key, label]) => (
            <button
              key={key}
              onClick={() => setView(key)}
              className={`px-3 py-1.5 rounded-md text-xs font-bold ${view === key ? "bg-white text-[#ff5a36] shadow-sm" : "text-slate-500"}`}
            >
              {label}
            </button>
          ))}
      ...
      {view === "results" ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <SummaryTile label="In Progress" value={summary.inProgress} color="text-[#ff5a36]" />
            ...
            {coursesForAct.map((c) => {
              ...
              return (
                <div key={c.id} className="p-4 flex items-center gap-4 hover:bg-slate-50 transition-colors">
                  <div className="w-10 h-10 rounded-lg bg-[#fff1ec] text-[#ff5a36] flex items-center justify-center flex-shrink-0">
                  ...
                  <button
                    ...
                    className="flex-shrink-0 flex items-center gap-1.5 bg-[#ff5a36] text-white text-xs font-bold px-3.5 py-2 rounded-lg hover:bg-[#c8431f]"
                  >
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <div className="flex items-start gap-3">
          <div className="w-11 h-11 rounded-lg bg-[#fff1ec] text-[#ff5a36] flex items-center justify-center flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-2.5">
              <div className="flex-1 bg-slate-100 rounded-full h-1.5 max-w-sm">
                <div className="bg-[#ff5a36] h-1.5 rounded-full transition-all" style={{ width: `${progressPercent}%` }} />
              </div>
              <span className="text-[11px] font-semibold text-slate-500 flex-shrink-0">
            ...
            {course.modules.map((m) => (
              <button
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-[11px] font-bold ${
                  ...
                  selectedModuleId === m.id ? "bg-[#ffe4da] text-[#c8431f]" : "bg-slate-100 text-slate-500"
                }`}
              >
                  ...
                  >
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-bold ${
                        m.is_complete ? "bg-green-100 text-green-700" : "bg-[#fff1ec] text-[#ff5a36]"
                      }`}
                    >
                      {m.is_complete ? <CheckCircle2 className="w-3.5 h-3.5" /> : m.sequence_order}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[9.5px] font-bold text-[#ff5a36] uppercase tracking-wide">Module {m.sequence_order}</div>
                  ...
                  {isExpanded && (
                    <div className="border-t border-slate-100 bg-slate-50/60 p-1.5 space-y-0.5">
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-2.5 py-2 rounded-md text-[11.5px] flex items-center gap-2 transition-colors ${
                              isActive ? "bg-white shadow-sm font-bold text-slate-800 ring-1 ring-[#ffe4da]" :
                              ci.is_unlocked ? "text-slate-600 hover:bg-white" : "text-slate-300 cursor-not-allowed"
                            }`}
                          >
                            <span className={`text-[9.5px] font-bold flex-shrink-0 w-14 whitespace-nowrap ${isActive ? "text-[#ff5a36]" : "text-slate-400"}`}>
                              Section {idx + 1}
                            </span>
                      ...
                      {m.assignments.map((a) => {
                        ...
                        return (
                          <div key={a.id} className={`mt-1 rounded-md border px-2.5 py-2 ${isActive ? "border-[#ffb495] bg-white" : "border-slate-200 bg-white"}`}>
                            ...
                            <button
                              ...
                              className={`mt-2 w-full flex items-center justify-center gap-1.5 text-[11px] font-bold px-3 py-1.5 rounded-md ${
                                m.is_complete ? "bg-[#ff5a36] text-white hover:bg-[#c8431f]" : "bg-slate-100 text-slate-400 cursor-not-allowed"
                              }`}
                            >
...
function ContentViewer({ content, sectionNumber, onComplete, marking }) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <div className="text-[10.5px] font-bold text-[#ff5a36] uppercase tracking-wide">Section {sectionNumber}</div>
      ...
      {content.is_completed ? (
        ...
        <div className="space-y-1.5">
          <button
            ...
            className="flex items-center gap-1.5 bg-[#ff5a36] text-white text-xs font-bold px-3 py-2 rounded-lg hover:bg-[#c8431f] disabled:opacity-40 disabled:cursor-not-allowed"
          >
