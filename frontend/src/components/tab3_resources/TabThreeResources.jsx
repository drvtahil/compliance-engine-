...
export default function TabThreeResources() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-slate-100 pb-3">
          <div>
            <h2 className="text-base font-bold text-slate-800 flex items-center gap-2">
              <FileText className="w-5 h-5 text-[#ff5a36]" /> Tab 3: Document &amp; Compliance Resources Library
            </h2>
          ...
          <button
            ...
            className="bg-[#ff5a36] hover:bg-[#c8431f] text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-xs transition self-start sm:self-auto"
          >
      ...
      {/* 2. SECTION TABS (left) + SEARCH & RESULTS (right) */}
      <div className="flex flex-col lg:flex-row items-start gap-4">
        <div className="w-full lg:w-56 flex-shrink-0 bg-white rounded-xl border border-slate-200 shadow-xs p-2">
          <div className="px-2 py-1.5 mb-1">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Sections</span>
          </div>
          <div className="space-y-0.5 lg:max-h-[560px] lg:overflow-y-auto">
            <button
        ...
        <div className="flex-1 min-w-0 w-full space-y-4">
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-white border border-slate-300 rounded-lg text-xs focus:ring-2 focus:ring-[#ff5a36] outline-hidden font-medium"
            />
          </div>
                    ...
                    >
                      <div>
                        <div className="flex justify-between items-start gap-2">
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            {doc.has_file && (
                              <button onClick={() => viewResourceFile(doc.id)} title="View Document" className="text-slate-400 hover:text-[#ff5a36] p-1 rounded hover:bg-slate-100 transition">
                            ...
                            <button onClick={() => handleOpenEditDoc(doc)} title="Edit Document" className="text-slate-400 hover:text-[#ff5a36] p-1 rounded hover:bg-slate-100 transition">
                      ...
                      <div className="pt-2 border-t border-slate-100 flex flex-wrap gap-1">
                        {doc.mapped_acts?.map((act, i) => (
                          <span key={`act-${i}`} className="text-[9px] font-bold bg-[#fff1ec] text-[#c8431f] px-1.5 py-0.5 rounded border border-[#ffe4da]">{act}</span>
                        ))}
              ...
              {visibleSections.map((sec) => {
                ...
                return (
                  <div key={sec.id} className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
                    {/* Section Header */}
                    <div className="bg-[#0b3f3c] text-white px-5 py-3.5 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                      <div className="flex items-center gap-2">
                        <FileText className="w-4 h-4 text-[#ff9269]" />
                        <h3 className="font-bold text-sm tracking-wide capitalize">{sec.name}</h3>
                        <span className="text-[11px] bg-[#12504c] text-[#d3ece8] px-2 py-0.5 rounded font-bold ml-1">
                        ...
                        {!sec.is_system && sec.name !== "Sample Policy" && (
                          <div className="flex items-center gap-1 ml-3 border-l border-[#17423b] pl-3">
                            <button
                              onClick={() => handleOpenEditSection(sec)}
                              title="Rename Section"
                              className="text-[#9fd0cb] hover:text-white p-1 rounded hover:bg-[#12504c]"
                            >
                              <Edit2 className="w-3.5 h-3.5" />
                            </button>
                            <button
                              ...
                              className="text-[#9fd0cb] hover:text-red-400 p-1 rounded hover:bg-[#12504c]"
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>
                          </div>
                      ...
                      <button
                        ...
                        className="bg-[#ff5a36] hover:bg-[#c8431f] text-white px-3.5 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-xs transition self-start sm:self-auto"
                      >
            ...
            <form onSubmit={handleSaveDocument} className="space-y-4 overflow-y-auto pr-1 flex-1">
              ...
              <div>
                ...
                {docForm.section_name === "Sample Policy" ? (
                  <select
                    ...
                    className="w-full border border-slate-300 rounded-lg p-2 text-xs bg-white font-semibold text-slate-800 focus:ring-2 focus:ring-[#ff5a36] outline-hidden"
                  >
                  ...
                  <input
                    ...
                    value={docForm.title}
                    onChange={(e) => setDocForm({ ...docForm, title: e.target.value })}
                    className="w-full border border-slate-300 rounded-lg p-2 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-[#ff5a36] outline-hidden"
                  />
                )}
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full border border-slate-300 rounded-lg p-2.5 text-xs text-slate-800 resize-y focus:ring-2 focus:ring-[#ff5a36] outline-hidden"
                />
              </div>
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                <label className="font-bold text-slate-700 block text-[11px] uppercase flex items-center gap-1.5">
                  <Upload className="w-3.5 h-3.5 text-[#ff5a36]" /> Upload Document (PDF, Excel, Word, etc.)
                </label>
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                <label className="font-bold text-slate-700 block text-[11px] uppercase flex items-center gap-1.5">
                  <BookOpen className="w-3.5 h-3.5 text-[#ff5a36]" /> 1. Acts (Multi-Select)
                </label>
            ...
            <form onSubmit={handleSaveSection} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-300 rounded-lg p-2 text-xs font-semibold focus:ring-2 focus:ring-[#ff5a36] outline-hidden"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="bg-[#ff5a36] hover:bg-[#c8431f] text-white px-4 py-1.5 rounded-lg font-bold flex items-center gap-1"
                >
                  {saving && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
